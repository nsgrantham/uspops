...
cat("Getting county population data for 2010-2018 ...\n")
...
tidy_up_county_pops <- function(county_pops_url) {
  read_csv(county_pops_url, col_types = cols(), progress = FALSE) %>%
    ...
    filter(year %in% 2:10) %>%  # guessing that these codes correspond to 2010, ..., 2018
...
get_county_pops_url <- function(state_code) {
  glue::glue("https://www2.census.gov/programs-surveys/popest/datasets/2010-2017/counties/asrh/cc-est2017-alldata-{state_code}.csv")
}
...
get_county_pops_url(pull(state_fips_except_puerto_rico, state_code)) %>%
  ...
  imap_dfr(function(url, state) {
    cat("+ Getting data for", state, "... ")
    county_pops <- tidy_up_county_pops(url)
    cat("Done.\n")
    county_pops
  }) %>%
  arrange(year, state, county, origin, race, sex, age) %>%
  write_tsv(file.path("data-raw", "county-pops-2010-2018.tsv"))
